...
class Project:
    def __init__(
        ...
        self.heap_size = [lambda: Default().get_heap_size(), lambda: heap_size][
            heap_size is not None
        ]()
        self.stack_size = [lambda: Default().get_stack_size(), lambda: stack_size][
            stack_size is not None
        ]()
        self.ecalls = ecalls
        self.ocalls = ocalls
        self.exit_addr = [
            lambda: Heuristic.find_exit(self.angr_project),
            lambda: exit_addr,
        ][exit_addr is not None]()
        self.enter_addr = [
            lambda: Heuristic.find_enter(self.angr_project),
            lambda: enter_addr,
        ...
        self.layout = EnclaveMemoryLayout(
            self.angr_project, self.heap_size, self.stack_size, self.old_sdk
        )

        self.init_enclave_state()
        self.entry_state.register_plugin("enclave", EnclaveState(self.angr_project))
        self.entry_state.register_plugin(
            "heap",
            angr.SimHeapBrk(
                heap_base=self.layout.heap_start, heap_size=self.layout.heap_size
            ),
        )
    ...
    def init_enclave_state(self):
        ...
        self.entry_state.mem[
            self.angr_project.loader.find_symbol("g_enclave_state").rebased_addr
        ].uint64_t = 2
