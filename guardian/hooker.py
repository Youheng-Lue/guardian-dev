...
class Hooker:
    def setup(self, proj, simgr, ecalls, ocalls, exited_addr, enter_addr, old_sdk):
        self.instruction_hooker(proj, self.instruction_replacement(exited_addr))
    ...
    def transitions_hooker(self, proj, ecalls, ocalls, exit_addr, enter_addr, old_sdk):
        ...
        proj.hook(enter_addr, hook=RegisterEnteringValidation(no_sanitisation=old_sdk))

    def instruction_replacement(self, exit_addr):
        def replace(capstone_instruction) -> angr.SimProcedure:
            if (
                capstone_instruction.mnemonic == "enclu"
                and capstone_instruction.address != exit_addr
            ):
                return SimEnclu()
