...
class EnclaveMemoryLayout:
    ...
    def round_to_page(self, size):
        return ((size) + ((self.SE_PAGE_SIZE) - 1)) & ~((self.SE_PAGE_SIZE) - 1)
    ...
    def __init__(self, project, heap_init_size, stack_max_size, old_sdk):
        ...
        self.stack_start = (
            self.heap_start
            + self.round_size_for_page(self.heap_size)
            + self.round_size_for_page(self.SE_GUARD_PAGE_SIZE)
        )
        self.stack_size = stack_max_size
        self.tcs_start = (
            self.stack_start
            + self.round_size_for_page(self.stack_size)
            + self.round_size_for_page(self.SE_GUARD_PAGE_SIZE)
        )
        self.tcs_size = self.TCS_SIZE
        self.ssa_start = self.tcs_start + self.round_size_for_page(self.tcs_size)
        self.ssa_size = self.size_from_page_count(self.SSA_FRAME_SIZE * self.SSA_NUM)
        self.td_start = (
            self.ssa_start
            + self.round_size_for_page(self.ssa_size)
            + self.round_size_for_page(self.SE_GUARD_PAGE_SIZE)
        )
        self.td_size = self.TD_SIZE
        self.enclave_size = self.td_start + self.round_size_for_page(self.td_size) + 1
        ...
        project.loader.memory.add_backer(
            self.heap_start,
            bytearray(
                self.td_start + self.round_size_for_page(self.td_size) - self.heap_start
            ),
        )

    def set_global_data(self, project, state):
        global_data_addr = project.loader.find_symbol("g_global_data").rebased_addr
        if self.old_sdk:
            # sdk version
            ...
            # heap offset
            state.mem[global_data_addr + 1 * 8].uint64_t = (
                self.heap_start - self.base_addr
            )
            # heap size
            ...
            # heap offset
            state.mem[global_data_addr + 2 * 8].uint64_t = (
                self.heap_start - self.base_addr
            )
            # heap size
    ...
    def set_thread_data(self, state):
        ...
        state.mem[
            self.td_start + 8
        ].uint64_t = self.stack_start + self.round_size_for_page(self.stack_size)
        # stack base relative to tcs in template but not here
        state.mem[
            self.td_start + 0x10
        ].uint64_t = self.stack_start + self.round_size_for_page(self.stack_size)
        # stack limit relative to tcs in template but not here
        state.mem[self.td_start + 0x18].uint64_t = (
            self.stack_start
            + self.round_size_for_page(self.stack_size)
            - self.stack_size
        )
